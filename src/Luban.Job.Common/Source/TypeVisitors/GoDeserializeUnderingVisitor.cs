...
{
    public class GoDeserializeUnderingVisitor : ITypeFuncVisitor<string, string, string,int, string>
    {
        ...
        public string Accept(TFshort type, string fieldName, string bufName ,string err,int depth)
        {
        ...
        public string Accept(TFint type, string fieldName, string bufName,string err,int depth)
        {
        ...
        public string Accept(TFlong type, string fieldName, string bufName ,string err,int depth)
        {
        ...
        public string Accept(TText type, string fieldName, string bufName ,string err,int depth)
        {
        ...
        public string Accept(TBytes type, string fieldName, string bufName ,string err,int depth)
        {
            return $"{{ if {fieldName}, {err} = {bufName}.ReadBytes(); {err} != nil {{ {err} = errors.New(\"{fieldName} error\"); return }} }}";
        }

        public string Accept(TVector2 type, string fieldName, string bufName,string err, int depth)
        {
            return $"{{ if {fieldName}, {err} = {bufName}.ReadVector2(); {err} != nil {{ {err} = errors.New(\"{fieldName} error\"); return }} }}";
        }

        public string Accept(TVector3 type, string fieldName, string bufName,string err, int depth)
        {
            return $"{{ if {fieldName}, {err} = {bufName}.ReadVector3(); {err} != nil {{ {err} = errors.New(\"{fieldName} error\"); return }} }}";
        }

        public string Accept(TVector4 type, string fieldName, string bufName ,string err,int depth)
        {
        ...
        {
            var __e = $"_e_{depth}";
            return $@"{{{fieldName} = make([]{elementType.Apply(GoTypeNameVisitor.Ins)}, 0); var _n_ int; if _n_, {err} = {bufName}.ReadSize(); {err} != nil {{ {err} = errors.New(""{fieldName} error""); return}}; for i := 0 ; i < _n_ ; i++ {{ var {__e} {elementType.Apply(GoTypeNameVisitor.Ins)}; {elementType.Apply(GoDeserializeBinVisitor.Ins, $"{__e}", bufName, err,depth+1)}; {fieldName} = append({fieldName}, {__e}) }} }}";
        }
        ...
        {
            return $@"{{ {fieldName} = make({type.Apply(GoTypeNameVisitor.Ins)}); var _n_ int; if _n_, {err} = {bufName}.ReadSize(); {err} != nil {{ {err} = errors.New(""{fieldName} error""); return}}; for i := 0 ; i < _n_ ; i++ {{ var _key_ {type.KeyType.Apply(GoTypeNameVisitor.Ins)}; {type.KeyType.Apply(GoDeserializeBinVisitor.Ins, "_key_", bufName, err,depth+1)}; var _value_ {type.ValueType.Apply(GoTypeNameVisitor.Ins)}; {type.ValueType.Apply(GoDeserializeBinVisitor.Ins, "_value_", bufName, err,depth+1)}; {fieldName}[_key_] = _value_}} }}";
        }
    }
}
