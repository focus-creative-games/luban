...
public static class DataLoaderUtil
{
    ...
    public static async Task<List<InputFileInfo>> CollectInputFilesAsync(IEnumerable<string> files, string dataDir)
    {
        var collectTasks = new List<InputFileInfo>();
        foreach (var file in files)
        {
            ...
            {
                var fileOrDirContent = await FileUtil.ReadAllBytesAsync(actualFullPath);
