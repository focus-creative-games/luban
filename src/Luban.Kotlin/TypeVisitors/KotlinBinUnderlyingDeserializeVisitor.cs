...
class KotlinBinUnderlyingDeserializeVisitor : ITypeFuncVisitor<string, string, int, string>
{
    ...
    public string Accept(TArray type, string bufName, string fieldName, int depth)
    {
        ...
        return $"run {{ val {__n} = {bufName}.readSize(); {fieldName} = Array({__n}) {{ {type.ElementType.Apply(KotlinDeclaringBoxTypeNameVisitor.Ins)}() }}; for ({__i} in 0 until {__n}) {{ var {__e}: {type.ElementType.Apply(KotlinDeclaringTypeNameVisitor.Ins)}; {type.ElementType.Apply(this, bufName, __e, depth + 1)}; {fieldName}[{__i}] = {__e} }} }}";
    }
