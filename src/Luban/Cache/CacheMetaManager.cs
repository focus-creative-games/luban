...
public class CacheMetaManager
{
    ...
    private static MetaInfo BuildMetaInfo(string file, string md5 = null)
    {
        var fullPath = Path.GetFullPath(file).Replace('\\', '/');
        if (md5 == null)
        {
            s_logger.Info("comput md5. file:{file}", file);
            md5 = FileUtil.CalcMD5(FileUtil.ReadAllBytes(file));
        }
        var fileInfo = new FileInfo(fullPath);
        long actualFileLength = fileInfo.Length;
        long actualLastModifiedTime = ((DateTimeOffset)fileInfo.LastWriteTime).ToUnixTimeMilliseconds();
    ...
    public string GetOrUpdateFileMd5(string file)
    {
        var meta = GetMetaInfo(file);

        if (meta == null)
        {
            meta = BuildMetaInfo(file);
            lock (_lock)
            {
                _dirty = true;
    ...
    public bool CheckFileChange(string file, string md5)
    {
        var meta = GetMetaInfo(file);
        if (meta == null)
        {
            ...
            s_logger.Debug("[add] meta not find, create it. file:{file} path:{path} md5:{md5} length:{length}", file, meta.FullPath, meta.Md5, meta.FileLength);
        }
        if (meta.Md5 != md5)
        {
            s_logger.Debug("[add] meta md5 not match,  file:{file} path:{path} md5:{md5} length:{length}", file, meta.FullPath, meta.Md5, meta.FileLength);
            return true;
        }
        return false;
    }

    public void UpdateFile(string file, string md5)
    {
        var meta = BuildMetaInfo(file, md5);
        lock (_lock)
        {
            _dirty = true;
            _cacheFileMetas[meta.FullPath] = meta;
