...
luban基于 **meta定义 + 数据源** 的设计，实现了**完备的类型系统**，增强了excel格式，同时提供json、xml、lua等丰富的数据源支持。既可以处理常规的简单配置，也完美处理**AI、技能、副本**等复杂配置，真正实现了统一数据定义、加载、检验、数据导出及代码生成的游戏配置工作流，彻底解决了中大型项目中需要程序员手动处理复杂配置的问题。
